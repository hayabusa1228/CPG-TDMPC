...
def evaluate(cfg: dict):
	...
	```
	"""
	print("noise_scale",cfg.noise_scale)
	...
	for task_idx, task in enumerate(tasks):
		...
		for i in range(cfg.eval_episodes):
			...
			if cfg.use_cpg:
				if obs[3]  > 0:
					# 左足前なら0で初期化
					cpg_states = torch.full((cfg.action_dim,), np.pi/2)
				else:
					# 右足前ならpiで初期化
					cpg_states = torch.full((cfg.action_dim,), -np.pi/2)
			if cfg.save_video:
			...
			while not done:
				# randam noiseを加える
				obs += cfg.noise_scale * torch.randn_like(obs)
				if cfg.use_cpg:
					# print("cpg_states",cpg_states)
					action, cpg_states = agent.act(obs, cpg_states, t0=t==0, eval_mode=True)
				else:
